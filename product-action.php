<?php
// Check if the "action" parameter is not empty in the GET request
if(!empty($_GET["action"])) 
{
    // Retrieve the product ID from the GET request and sanitize it
    $productId = isset($_GET['id']) ? htmlspecialchars($_GET['id']) : '';
    // Retrieve the quantity from the POST request and sanitize it
    ...
    switch($_GET["action"])
    {
                ...
                if(!empty($_SESSION["cart_item"])) 
                {
                    ...
                    if(in_array($productDetails->d_id, array_keys($_SESSION["cart_item"]))) 
                    {
                        ...
                        foreach($_SESSION["cart_item"] as $k => $v) 
                        {
                            ...
                            {
                                // Initialize quantity if it's empty
        ...
        case "empty":
            // Unset the cart session variable to empty the cart
